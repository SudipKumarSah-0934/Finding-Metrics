...
public class Graph2 {
	...
	private HashMap<Character, ArrayList<Character>> visitTree(Node root, ArrayList<Node> visited,
		...
		for (int i = 0; i < childs.size(); i++) {
			...
			if (!visited.contains(current))
				visitTree(current, visited, table);
		}
			...
			if (this.lines.get(lineNo).contains("else if")) {
				...
				if (statement_node == null) {
					...
					if (!lastNodesInBranch.isEmpty()) {
						...
						parent.childs.add(statement_node);
				...
				if (lines.get(lineNo).contains("}") || !isMultipleLine)
	...
	private boolean isBlock(Node newNde) {
		if (this.lines.get(lineNo).endsWith("{"))
			return true;
		lineNo++;
		while (true) {
			if (this.lines.get(lineNo).endsWith("{"))
